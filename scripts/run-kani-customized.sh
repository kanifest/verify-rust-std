...
main() {
    ...
    if [[ "$run_command" == "verify-std" ]]; then
        if [[ -n "$WORKER_INDEX" && -n "$WORKER_TOTAL" ]]; then
            ...
            echo "Command: $kani_path verify-std -Z unstable-options ./library $unstable_args $command_args --enable-unstable --cbmc-args --object-bits 12"
            "$kani_path" verify-std -Z unstable-options ./library \
                $unstable_args \
                $command_args \
                --enable-unstable \
        ...
        echo "Command: $kani_path list -Z list $unstable_args ./library --std --format markdown"
